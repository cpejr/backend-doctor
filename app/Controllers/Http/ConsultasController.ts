...
export default class ConsultasController {
  ...
  public async store({ request }: HttpContextContract) {
    ...
    const duracao_em_minutos = request.input('duracao_em_minutos')
    const descricao = request.input('descricao')
    const avaliacao = request.input('avaliacao')
  ...
  public async update({ request }: HttpContextContract) {
    ...
    const consultaData = {
      ...
      duracao_em_minutos: request.input('duracao_em_minutos'),
    ...
    consulta.merge(limpaCamposNulosDeObjeto(consultaData))
    await consulta.save()
